...
const Navbar = () => {
    ...
    const [checkingPremium, setCheckingPremium] = useState(true); // to wait for user info
    ...
    useEffect(() => {
        const checkPremiumStatus = async () => {
            ...
            try {
                ...
                const { data: userInfo } = await axiosSecure.get(`/users/${encodeURIComponent(user.email)}`);
        ...
        <div className="navbar bg-base-100 shadow-sm">
            ...
            <div className="navbar-center hidden lg:flex">
                <ul className="menu menu-horizontal px-1">
                    ...
                    <li>
                        <NavLink to="/addArticle" className={({ isActive }) => isActive ? "text-green-700 font-bold" : ""}>Add Articles</NavLink>
                    </li>
                    <li>
                        <NavLink to="/allArticle" className={({ isActive }) => isActive ? "text-green-700 font-bold" : ""}>All Articles</NavLink>
                    </li>
                    <li>
                        <NavLink to="/subscription" className={({ isActive }) => isActive ? "text-green-700 font-bold" : ""}>Subscription</NavLink>
                    </li>
                    <li>
                        <NavLink to="/profile" className={({ isActive }) => isActive ? "text-green-700 font-bold" : ""}>My Profile</NavLink>
                    </li>
                    {isPremium && !checkingPremium && (
                        <li>
                            <NavLink to="/PremiumArticles" className={({ isActive }) => isActive ? "text-green-700 font-bold" : ""}>Premium Articles</NavLink>
                        </li>
                    )}
                    {user && (
                        <li>
                            <NavLink to="/dashboard" className={({ isActive }) => isActive ? "text-green-700 font-bold" : ""}>Dashboard</NavLink>
                        </li>
                    )}
                </ul>
            </div>
